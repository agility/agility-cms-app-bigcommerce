...
export default function ProductRow({ product, node, onSelectProduct }: Props) {
  return (
    <div className="flex flex-row items-center rounded shadow border border-gray-100 p-2 gap-3">
      <div className="flex-shrink-0">
        <img src={product.image.listingUrl} className="h-13 w-13 rounded" />
      </div>
      ...
      {product.sku && (
        <div
          className="text-sm text-gray-500 flex-nowrap line-clamp-1 w-32 break-all"
          title={product.sku}
        >
          SKU: {product.sku}
        </div>
      ...
      <div className="">
        <button
          ...
          className={classNames(
            "inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-purple-500 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
          )}
          onClick={() => {
